...
EOBODY;
    ...
    /* Setting query based on SearchFilters.php */
    $query = "select * from " . $table . " where ";

    if (!empty($_POST['breed'])){
        $query .= "breed = \"{$_POST['breed']}\" and ";
    }
    if (!empty($_POST['gender'])){
        $query .= "gender = \"{$_POST['gender']}\" and ";
    }
    
    $query .= "age between \"{$_POST['ageLow']}\" and \"{$_POST['ageHigh']}\"";
    ...
    if (!$result) {
        ...
        if ($num_rows === 0) {
            ...
            echo "<script>alert('No Pets in Database with the selected search filters');";
            echo 'window.location.href = "SearchFilters.php";</script>';
            ...
            for ($i = 0; $i < count($_SESSION['pets']); $i++){
                ...
                /* Returns the an array with long lat city state */
                /* API only allows 100 calls per day */
                $distance = getLongAndLat($_SESSION['pets'][$i]['zipcode']);
                $_SESSION['pets'][$i]['long'] = $distance['long'];
                $_SESSION['pets'][$i]['lat'] = $distance['lat'];
                $_SESSION['pets'][$i]['city'] = $distance['city'];
                $_SESSION['pets'][$i]['state'] = $distance['state'];

                
                if ($distance > $_POST['distance']){
                     /* Remove pets that are further than the search filter distance */
                    unset($_SESSION['pets'][$i]);
            ...
            // for ($i = 0; $i < count($_SESSION['pets']); $i++){
            //     if(isset($_SESSION['pets'][$i])){
            //     echo $_SESSION['pets'][$i]['dogName'] . "<br>";
            //     }
            // }
            /*-----------------------------------------------------------*/
